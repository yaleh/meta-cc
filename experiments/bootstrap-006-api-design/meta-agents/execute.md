...
build_context(agent, P) = {
  ...
  inputs: {
    data_sources: [api_schemas, usage_patterns, user_feedback],
...
agent_coordination = {
  data-analyst: {
    inputs: [api_schemas, usage_data, user_feedback],
    tasks: ["statistics", "pattern_analysis", "metrics"],
    outputs: [metrics, analysis, reports]
  },

  doc-writer: {
    inputs: [analysis_results, iteration_data, api_specs],
    tasks: ["documentation", "guidelines", "methodology"],
    outputs: [markdown_files, guides, templates]
  },

  coder: {
    inputs: [requirements, api_patterns, test_cases],
    tasks: ["tools", "validators", "automation"],
    outputs: [code_files, tests, documentation]
  },

  # Specialized agents (created during execution if needed)
  api-consistency-checker: {
    inputs: [api_schemas, usage_patterns],
    tasks: ["consistency_analysis", "violation_detection"],
    outputs: [consistency_report, recommendations]
  },

  parameter-designer: {
    inputs: [tool_schemas, usage_patterns, user_feedback],
    tasks: ["parameter_analysis", "design_patterns"],
    outputs: [design_guidelines, parameter_standards]
  }
}
...
collect(E) = {
  api_analysis: [consistency_reports, usage_statistics, pattern_analysis],
  documentation: [iteration_reports, design_guidelines, methodologies],
  tools: [validators, consistency_checkers, automation_code],
  data: [processed_data, metrics, api_assessments]

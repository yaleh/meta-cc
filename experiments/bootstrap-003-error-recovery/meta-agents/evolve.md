...
agent_types :: Error_Domain → Agent_Template
agent_types(domain) = {
  classification: {
    name: "error-classifier",
    when: needs_error_taxonomy,
    capabilities: [develop_taxonomy, define_criteria, classify_consistently],
    value_impact: V_detection ↑
  },

  diagnosis: {
    name: "root-cause-analyzer",
    when: diagnosis_accuracy_low,
    capabilities: [trace_root_causes, develop_methodology, create_tools],
    value_impact: V_diagnosis ↑
  },

  recovery: {
    name: "recovery-advisor",
    when: recovery_procedures_missing,
    capabilities: [design_strategies, document_procedures, evaluate_effectiveness],
    value_impact: V_recovery ↑
  },

  prevention: {
    name: "error-pattern-learner",
    when: recurring_errors_high,
    capabilities: [recognize_patterns, build_models, suggest_preventions],
    value_impact: V_prevention ↑
  }
}
...
meta_evolution_triggers = {
  error_triage: {
    trigger: needs_severity_based_prioritization,
    capability: "triage_by_severity",
    integration: extends(plan),
    rare: true  # Most experiments don't need this
  },

  real_time_monitoring: {
    trigger: needs_continuous_observation,
    capability: "monitor_continuously",

...
- maintain_conciseness: length(refined) ≤ 3 × length(original)

output :: Refinement → Analysis_Report
output(R) = original(scored) ∧ gaps(diagnosed) ∧ refined(scored) ∧ improvements(explained) ∧ usage(guided)

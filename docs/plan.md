...
## Phase 22: 统一 Meta 命令与多源能力发现（Unified Meta Command & Multi-Source Capability Discovery）

**目标**：实现统一 `/meta` 命令，支持从多个源动态加载能力，通过语义匹配自动选择和组合能力执行
**代码量**：~800 行 | **优先级**：高 | **状态**：🚧 进行中

### 背景

当前有 13 个独立 slash commands（meta-errors、meta-quality-scan 等），维护成本高，用户需记忆多个命令。通过统一入口 `/meta` 和动态能力发现系统，实现：
1. 用户使用自然语言描述意图
2. MCP 从多个源（本地目录、GitHub 仓库）加载能力
3. Claude 语义匹配并自动选择/组合能力
4. 支持本地开发、fork 测试、社区扩展

### 架构设计

```
用户输入: /meta "show errors"
  ↓
MCP: list_capabilities() 从多源加载能力索引
  源: ["~/dev/caps", "yaleh/meta-cc", "community/extra"]
  ↓
Claude: 语义匹配
  匹配: meta-errors (score: 5)
  ↓
MCP: get_capability(name="meta-errors") 获取完整实现
  ↓
Claude: 执行能力实现
  ↓
输出: 错误分析报告（内联可见）
...
### Stage 22.1: MCP 多源能力发现基础（~200 行，3h）
- 实现环境变量 `META_CC_CAPABILITY_SOURCES="source1:source2"`
- 支持本地路径和 GitHub repo 自动检测
- 实现源优先级合并逻辑（同名能力覆盖）
- 添加 frontmatter 解析（name, description, keywords, category）
- **交付**：`cmd/mcp-server/capabilities.go` (+200), `capabilities_test.go` (+150)

### Stage 22.2: MCP 能力索引工具（~150 行，2h）
- 实现 MCP 工具 `list_capabilities()`
- 支持本地缓存（1小时 TTL，本地源无缓存）
- 实现隐藏测试参数 `_sources`, `_disable_cache`
- 返回紧凑能力索引 JSON
- **交付**：`cmd/mcp-server/tools.go` (+50), `executor.go` (+30), 集成测试 (+70)

### Stage 22.3: MCP 能力获取工具（~100 行，1.5h）
- 实现 MCP 工具 `get_capability(name)`
- 从多源查找能力（按优先级）
- 返回完整 .md 文件内容
- 支持本地和 GitHub 源
- **交付**：`cmd/mcp-server/capabilities.go` (+80), 测试 (+80)

### Stage 22.4: 扩展现有 Slash Commands Frontmatter（~80 行，1.5h）
- 为所有 `.claude/commands/meta-*.md` 添加元数据
- 添加 `keywords` 字段（逗号分隔）
- 添加 `category` 字段（diagnostics, assessment, visualization 等）
- 验证 frontmatter 格式正确
- **交付**：更新 13 个 .md 文件 frontmatter

### Stage 22.5: 统一 /meta Slash Command（~200 行，3h）
- 创建 `.claude/commands/meta.md`
- 实现能力发现流程（调用 MCP list_capabilities）
- 实现语义匹配逻辑（关键词评分）
- 实现单一能力执行（调用 get_capability + 执行步骤）
- 处理无匹配情况（显示可用能力列表）
- **交付**：`.claude/commands/meta.md` (+200)

### Stage 22.6: 组合能力执行（~100 行，2h）
- 实现多能力检测（多个高分匹配）
- 实现组合执行模式（pipeline: 数据 → 可视化）

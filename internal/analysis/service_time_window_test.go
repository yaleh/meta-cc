...
func TestAnalysisTools_WindowedCountsExcludeOutOfWindowData(t *testing.T) {
	...
	t.Run("analyze_bugs", func(t *testing.T) {
		// Decode the service's wire output, not analyzer.BugAnalysisResult: DIR-096
		// promoted each pattern's `examples` from bare strings to objects, so the
		// analyzer's own struct (which stays []string -- the reshape happens at
		// this boundary) no longer unmarshals it. Only the counts below are
		// asserted here, so a local view of those is the honest decode target.
		type bugWireView struct {
			Patterns    []map[string]interface{} `json:"patterns"`
			TotalPairs  int                      `json:"total_pairs"`
			TotalErrors int                      `json:"total_errors"`
		}

		var fullRes bugWireView
